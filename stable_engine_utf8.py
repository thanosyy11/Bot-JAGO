...
class SiliwangiEngine:
    ...
    def _simpan_snapshot_html(self, html_text, nama_kejadian):
        try:
            ...
            logger.info(f"≡ƒô╕ Screenshot Tersimpan: '{nama_file}'")
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if res.headers.get('cf-mitigated') or "Just a moment" in res.text[:300] or "Checking your browser" in res.text[:300]:
                    logger.error(f"≡ƒÜ¿ [{self.username}] Terblokir Cloudflare di {url}!")
                    raise CloudflareBlockException("Terblokir oleh Cloudflare.")

                if res.status_code == 403:
                    logger.error(f"≡ƒÜ¿ [{self.username}] Bot diblokir server (403 Forbidden).")
                    raise CloudflareBlockException("Diblokir Server (403).")

                if res.status_code == 429:
                    retry_after = int(res.headers.get("Retry-After", 5))
                    logger.warning(f"ΓÜá∩╕Å [{self.username}] Rate limited (429). Tunggu {retry_after} detik.")
    ...
    async def _safe_request_with_recovery(self, method, url, max_retries=4, **kwargs):
        ...
        try:
            ...
            if res and res.status_code in [301, 302, 401]:
                logger.warning(f"≡ƒöä [{self.username}] Terdeteksi error {res.status_code}, coba reconnect...")
                ...
                if self.reconnect_attempt < self.max_reconnect_attempts:
                    ...
                    logger.info(f"≡ƒöä [{self.username}] Reconnect attempt {self.reconnect_attempt}/{self.max_reconnect_attempts}...")

                    if await self.login():
                        logger.info(f"Γ£à [{self.username}] Reconnected successfully, retrying request...")
                        # Retry sekali saja
                        res = await self._safe_request(method, url, max_retries=2, **kwargs)
                        return res
                    else:
                        logger.error(f"Γ¥î [{self.username}] Re-login failed")
                        return res
    ...
    def _load_saved_cookies(self):
        ...
        if saved:
            ...
            logger.info(f"≡ƒöæ [{self.username}] Cookies dimuat dari DB.")
    ...
    def _session_is_valid(self) -> bool:
        ...
        logger.debug(f"≡ƒöì [{self.username}] Validating session freshness...")
    ...
    def _save_cookies(self):
        ...
        try:
            ...
            if cookies_list:
                ...
                logger.info(f"≡ƒÆ╛ [{self.username}] {len(cookies_list)} cookies disimpan ke DB (session fresh).")
        except Exception as e:
            logger.warning(f"ΓÜá∩╕Å Gagal simpan cookies [{self.username}]: {e}")
    ...
    async def login(self):
        ...
        try:
            ...
            if "Keluar" in response.text or "Logout" in response.text or "Pesanan" in response.text:
                logger.info(f"Γ£à [{self.username}] Sesi masih aktif (cookies).")
                self._save_cookies()
                return True

            # Session expired ΓåÆ clear lama, fresh login
            logger.info(f"≡ƒöÉ [{self.username}] Session expired, login ulang...")
            clear_session_cookies(self.telegram_id, self.username)
            ...
            if login_res and ("Keluar" in login_res.text or "Logout" in login_res.text):
                logger.info(f"Γ£à Login sukses (fresh): {self.username}")
                self._save_cookies()
                return True
            else:
                logger.warning(f"Γ¥î Login gagal: {self.username}")
                if login_res:
    ...
    async def _validate_kelipatan(self, keranjang):
        ...
        logger.info(f"≡ƒº« [{self.username}] Cek Draf: MAXI={total_maxi}, DC={total_dc}")

        aman = True
        if total_maxi > 0 and total_maxi % 12 != 0:
            logger.error(f"Γ¥î [{self.username}] MAXI DITOLAK! {total_maxi} bukan kelipatan 12.")
            aman = False
        if total_dc > 0 and total_dc % 4 != 0:
            logger.error(f"Γ¥î [{self.username}] DC DITOLAK! {total_dc} bukan kelipatan 4.")
            aman = False
        if aman:
            logger.info(f"Γ£à [{self.username}] Aturan kelipatan OK.")
        return aman
    ...
    async def clear_cart(self):
        logger.info(f"≡ƒº╣ [{self.username}] Membersihkan keranjang...")
        try:
            ...
            if not remove_links:
                logger.info(f"Γ£¿ [{self.username}] Keranjang sudah bersih.")
                return
            for link in remove_links:
                href = link.get('href')
                if href:
                    await self._safe_request('GET', href)
            logger.info(f"≡ƒùæ∩╕Å [{self.username}] Dihapus {len(remove_links)} item sisa.")
        except Exception as e:
            logger.error(f"Gagal bersihkan keranjang {self.username}: {e}")

    # ------------------------------------------------------------------
    # ADD TO CART
    # Γ£à Menggunakan ?wc-ajax=add_to_cart (standard WooCommerce AJAX)
    # ------------------------------------------------------------------
    ...
    async def _add_to_cart(self, prod_id, qty) -> tuple:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Γ¥î [{self.username}] Respons add_to_cart bukan JSON. Kemungkinan redirect.")
                return False, 0
            ...
            try:
                ...
                # Server menolak ΓÇö cek apakah ada stok parsial
                if data.get('error'):
                    err_txt = str(data.get('error', ''))
                    logger.warning(f"≡ƒò╡∩╕Å [STOK] ID {prod_id} ditolak: {err_txt[:120]}")
                    available = self._parse_available_stock(err_txt)
                    if available and 0 < available < qty:
                        logger.info(f"≡ƒôª Stok parsial terdeteksi: {available}x tersedia untuk ID {prod_id}")
    ...
    async def add_to_cart_with_fallback(self, item):
        ...
        if success:
            logger.info(f"Γ£à [{self.username}] Masuk: {added}x {nama}")
            return True

        if target_tier == 0:
            logger.error(f"Γ¥î [{self.username}] {nama} HABIS (Tier 0, no fallback).")
            return False

        logger.warning(f"ΓÜá∩╕Å [{self.username}] {nama} HABIS! Mencari pengganti...")
        conn = sqlite3.connect(DB_NAME, timeout=10)
        cursor = conn.cursor()
        cursor.execute('''
        ...
        for alt_id, alt_nama, alt_tier in alternatives:
            logger.info(f"   ≡ƒöä [{self.username}] Mencoba: {alt_nama} (Tier {alt_tier})...")
            ok, added = await self._add_to_cart(alt_id, qty)
            if ok:
                logger.info(f"   ≡ƒÄ» [{self.username}] Disubstitusi dengan: {alt_nama}!")
                return True

        logger.error(f"≡ƒÆÇ [{self.username}] GAGAL TOTAL! Semua varian {kategori} habis.")
        return False
    ...
    async def _smart_maxi_fill(self, maxi_items: list) -> int:
        ...
        - Tier chain: Tier 1 ΓåÆ Tier 2 ΓåÆ Tier 3
        ...
        # ΓöÇΓöÇ Bangun priority queue ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Proses setiap produk dalam antrian ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        for product in priority_queue:
            ...
            # Tier 3 cut ΓÇö hanya 1x, hanya jika Tier 1 & 2 tidak berkontribusi
            if (tier == 3
                    and not tier3_cut_applied
                    and tiers_contributed.isdisjoint({1, 2})):
                cut = max(12, (int(remaining * 0.60) // 12) * 12)
                logger.info(f"Γ£é∩╕Å [{self.username}] Tier 3 only ΓÇö potong 40%: {remaining}x ΓåÆ {cut}x")
                self._step("Γ£é∩╕Å", "Tier 3 Only",
                           f"Semua T1+T2 habis. Target dipotong 40%: {remaining}ΓåÆ{cut}x")
                remaining = cut
            ...
            if added > 0:
                ...
                remaining   -= added
                if added < qty_to_try:
                    self._step("ΓÜí", f"{product['nama']}",
                               f"Parsial {added}/{qty_to_try}x (sisa {remaining}x)")
                else:
                    self._step("≡ƒ¢Æ", f"{added}x {product['nama']}", "Masuk")
            else:
                self._step("ΓÜá∩╕Å", f"{product['nama']}", "Habis / Gagal")

        if remaining > 0:
            logger.warning(f"ΓÜá∩╕Å [{self.username}] MAXI kurang {remaining}x dari target {total_needed}x")
            self._step("ΓÜá∩╕Å", "MAXI", f"Kurang {remaining}x dari {total_needed}x")

        logger.info(f"Γ£à [{self.username}] Total MAXI masuk keranjang: {total_added}x")
        return total_added
    ...
    async def get_checkout_nonce(self):
        ...
        1. woocommerce-process-checkout-nonce  ΓåÆ untuk POST ?wc-ajax=checkout
        2. security nonce                       ΓåÆ untuk POST ?wc-ajax=update_order_review
        """
        try:
            ...
            if res.status_code in [301, 302]:
                ...
                logger.error(f"Γ¥î [{self.username}] Cart kosong atau session expired, redirect ke {loc}")
                return False
            ...
            if self.security_nonce:
                logger.info(f"≡ƒöÉ [{self.username}] Security nonce OK: {self.security_nonce[:8]}...")
            else:
                logger.warning(f"ΓÜá∩╕Å [{self.username}] Security nonce tidak ditemukan, update_order_review mungkin gagal.")
    ...
    def _extract_security_nonce(self, html_text):
        ...
        TIDAK menggunakan pattern "nonce" generik ΓÇö terlalu berisiko ambil nonce yang salah.
        """
        # ΓöÇΓöÇ Strategi 1: Isolasi blok wc_checkout_params ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Strategi 2: Cari di seluruh HTML (lebih luas) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Strategi 3: Hidden input[name=security] ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # Γ£à Sesuai record: POST ke ?wc-ajax=update_order_review
    # ------------------------------------------------------------------
    ...
    async def _call_update_order_review(self, base_payload):
        ...
        if not self.security_nonce:
            logger.warning(f"ΓÜá∩╕Å [{self.username}] Melewati update_order_review (nonce tidak ada).")
            return False
        ...
        try:
            ...
            if res and res.status_code == 200:
                logger.info(f"Γ£à [{self.username}] update_order_review berhasil (200 OK).")
                return True
            else:
                status = res.status_code if res else "No Response"
                logger.warning(f"ΓÜá∩╕Å [{self.username}] update_order_review status: {status}")
                return False
        except Exception as e:
    ...
    async def execute_order(self):
        ...
        self.step_log = []
        self.reconnect_attempt = 0
        self._step("≡ƒöæ", "Memulai", "WAR")

        logger.info(f"≡ƒöæ [{self.username}] Mengamankan sesi login...")
        if not await self.login():
            self._step("Γ¥î", "Login", "GAGAL")
            logger.error(f"≡ƒ¢æ [{self.username}] Gagal login!")
            return False
        self._step("Γ£à", "Login", "Sesi aktif")
        ...
        if not row:
            logger.error(f"≡ƒ¢æ [{self.username}] Draf KOSONG dari database saat eksekusi!")
            return False
        ...
        if not await self._validate_kelipatan(keranjang):
            logger.error(f"≡ƒ¢æ [{self.username}] Draf ditolak sebelum masuk keranjang.")
            return False

        await self.clear_cart()
        self._step("≡ƒº╣", "Clear Cart", "Selesai")

        # ΓöÇΓöÇ Pisahkan item berdasarkan kategori ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ DC & PLASTIK: Tambah secara PARALEL untuk kecepatan ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        async def _add_item_task(item, fallback_msg):
            ok, added = await self._add_to_cart(item['id'], item['qty'])
            if ok:
                self._step("≡ƒ¢Æ", f"{added}x {item['nama']}", "Masuk")
            else:
                self._step("ΓÜá∩╕Å", f"{item['qty']}x {item['nama']}", fallback_msg)
        ...
        if not await self.get_checkout_nonce():
            self._step("Γ¥î", "Checkout Nonce", "Tidak ditemukan")
            return False
        self._step("≡ƒöÉ", "Checkout Nonce", f"{self.checkout_nonce[:8]}..." if self.checkout_nonce else "N/A")
        self._step(
            "≡ƒöÉ" if self.security_nonce else "ΓÜá∩╕Å",
            "Security Nonce",
        ...
        try:
            ...
            if cart_res:
                ...
                if error_notices:
                    ...
                    logger.error(f"Γ¥î [{self.username}] Error di keranjang: {pesan}")
                    self._simpan_snapshot_html(cart_res.text, "Error_Di_Keranjang")
                    return False

                cart_items = soup_cart.find_all('tr', class_='cart_item')
                if not cart_items:
                    logger.error(f"Γ¥î [{self.username}] KERANJANG KOSONG setelah add_to_cart!")
                    return False
            ...
            if not res or "checkout" not in str(res.url):
                logger.error(f"Γ¥î [{self.username}] Terpental dari kasir: {res.url if res else 'N/A'}")
                if res:
            ...
            # 4) Γ£à Panggil update_order_review sebelum checkout final (sesuai record)
            await self._call_update_order_review(base_payload)
            self._step(
                "Γ£à" if self.security_nonce else "ΓÜá∩╕Å",
                "update_order_review",
                "Berhasil" if self.security_nonce else "Dilewati (nonce tidak ada)"
            )

            # 5) POST checkout final ΓÇö hanya metode 'cheque' (sesuai record)
            checkout_url = "https://siliwangibolukukus.com/?wc-ajax=checkout"

            logger.info(f"≡ƒÆ│ [{self.username}] Mencoba checkout via CHEQUE...")
            ajax_headers = {"X-Requested-With": "XMLHttpRequest", "Accept": "application/json"}
            final_res = await self._safe_request_with_recovery('POST', checkout_url, headers=ajax_headers, data=base_payload)
            if not final_res:
                logger.error(f"≡ƒÆÇ [{self.username}] Tidak ada respons dari server checkout.")
                return False
            ...
            try:
                ...
                if result.get('result') == 'success':
                    logger.info(f"≡ƒÄë [{self.username}] Checkout BERHASIL via JSON response!")
                    redirect_url = result.get('redirect', '')
                    m = re.search(r'/order-received/(\d+)/', redirect_url)
                    self.order_id_woo = m.group(1) if m else "UNKNOWN"
                    logger.info(f"≡ƒöû [{self.username}] Order ID diekstrak: {self.order_id_woo}")
                    try:
                        self._mark_success()
                    except Exception as e:
                        logger.warning(f"ΓÜá∩╕Å Gagal membersihkan draf setelah sukses: {e}")
                    return True
                elif result.get('result') == 'failure':
                    messages = result.get('messages', '')
                    logger.warning(f"ΓÜá∩╕Å [{self.username}] Checkout DITOLAK: {messages[:200]}")
            ...
            if "order-received" in str(final_res.url):
                logger.info(f"≡ƒÄë [{self.username}] Checkout BERHASIL via redirect URL!")
                m = re.search(r'/order-received/(\d+)/', str(final_res.url))
                self.order_id_woo = m.group(1) if m else "UNKNOWN"
                logger.warning(f"ΓÜá∩╕Å Redirect history: {len(final_res.history)} redirects. Order ID: {self.order_id_woo}")
                self._mark_success()
                return True

            # Cek sukses via teks HTML
            if "Pesanan" in final_res.text or "Order Complete" in final_res.text:
                logger.info(f"≡ƒÄë [{self.username}] Checkout BERHASIL via HTML text!")
                self._mark_success()
                return True

            logger.error(f"≡ƒÆÇ [{self.username}] Checkout GAGAL. Response: {final_res.text[:200]}")
    ...
    # ------------------------------------------------------------------
    # MARK SUCCESS ΓÇö hapus draft, simpan ke history
    # ------------------------------------------------------------------

    def _mark_success(self):
        """
        Γ£à Perbaikan: Draft benar-benar DIHAPUS dari draft_orders setelah sukses,
        ...
        conn.commit()
        conn.close()
        logger.info(f"Γ£à [{self.username}] Draft dihapus, riwayat tersimpan.")
